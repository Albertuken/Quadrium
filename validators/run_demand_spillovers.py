...
figure. 2018:
    ...
    jobs                   220     5.1 %    10.8 %    21.3 %
    at the surveys' level          output 19.8 %, jobs 19.8 % (medians)
...
The figures live in `EVIDENCE['demand_spillover_pct']`, which the report
quotes, and this file fails if the two disagree. It also requires the
...
def main() -> int:
    ...
    def measure(Zm, cols):
        ...
        return out, jobs[np.isfinite(jobs)], es

    out, jobs, es = measure(Z, five)
    check("weighted by the final demand for its products, a region's output "
          ...
          f"p90 {pct(out, 90)} %")
    check("and its jobs a median 10.8 %",
    ...
    out4, jobs4, es4 = measure(Z4, five)
    check("at the surveys' level of domestic trade, 19.8 % and 19.8 %",
    ...
    try:
        ...
        ok, detail = gap < 1e-9, f"largest difference {gap:.1e}"
