#!/usr/bin/env python3
"""
IO Model Foundry — run a disaggregation from a configuration workbook.

No Python required. Fill in a spreadsheet, run one command, read the report.

    python3 run_quadrium.py --template my_config.xlsx    # get a blank workbook
    python3 run_quadrium.py my_config.xlsx               # run it
    python3 run_quadrium.py my_config.xlsx --check       # validate, do not run
...
def main(argv=None) -> int:
    ...
    if args.template:
        ...
        print(f"    python3 run_quadrium.py {p}")
        return 0
